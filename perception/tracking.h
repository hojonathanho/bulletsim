...
vector<bool> checkOccluded(const vector<btVector3>& xyzs_world, const pcl::PointCloud<pcl::PointXYZRGB>::Ptr& cloud,  Eigen::Affine3f cam2world);
MatrixXi xyz2uv(const Matrix<float,Dynamic,3>& xyz);
Eigen::Affine3f getCam2World(const vector<Vector3f>& tableVerts, Vector3f& normal, float scale=1);
MatrixXi xyz2uv(const Matrix<float,Dynamic,3>& xyz);
