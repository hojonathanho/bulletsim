...
void			btSoftBody::randomizeConstraints(float* tex_coords)
{
	unsigned long	seed=243703;
#define NEXTRAND (seed=(1664525L*seed+1013904223L)&0xffffffff)
	int i,ni;
	...
	{
		int ri = NEXTRAND%ni;
		btSwap(m_faces[i],m_faces[ri]);
		btSwap(tex_coords[i*6],tex_coords[ri*6]);
		btSwap(tex_coords[i*6+1],tex_coords[ri*6+1]);
		btSwap(tex_coords[i*6+2],tex_coords[ri*6+2]);
		btSwap(tex_coords[i*6+3],tex_coords[ri*6+3]);
		btSwap(tex_coords[i*6+4],tex_coords[ri*6+4]);
		btSwap(tex_coords[i*6+5],tex_coords[ri*6+5]);
	}
#undef NEXTRAND
}
