...
SoftBodyNodePicker::SoftBodyNodePicker(Scene &scene_, osg::Camera *cam_, btSoftBody *psb_) : scene(scene_), cam(cam_), psb(psb_) {
    scene.addCallback(osgGA::GUIEventAdapter::RELEASE,
            boost::bind(&SoftBodyNodePicker::pickCallback, this, _1));
}

static const btScalar RAYCAST_DIST = 10000;
bool SoftBodyNodePicker::pickCallback(const osgGA::GUIEventAdapter &ea) {
    ...
    btVector3 from = util::toBtVector(look) /*+ ( up * ( _launchModel->getBound()._radius * 2. ) )*/;
    ...
    if (res.feature == btSoftBody::eFeature::Face) {
        // get some node on the face
        pickCb(psb->m_faces[res.index].m_n[0]);
    }
