...
// vector<btTransform> findBasePositions(const vector<btTransform>& leftPoses, vector<bool> grabbing) {
//   vector<btTransform> base_tfs;
//   vector<grabbing, vector<btTransform> > splitPoses = splitby(leftPoses, grabbing);
//   for (int i=0; i < splitPoses.size(); i++) {
//     bool g = splitPoses[i].first;
//     vector<btTransform> poses = splitPoses[i].second;
//     vector<btTransform> new_base_tfs = g ? 
//   }
// }
