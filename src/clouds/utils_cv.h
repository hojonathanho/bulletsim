...
cv::Mat backgroundSubtractorDepthMask(cv::Mat depth, std::vector<cv::Mat> depth_bg, double depth_th=0.004, int erode=1, int dilate=2, int min_connected_components=300);
...
cv::Mat backgroundSubtractorColorMask(cv::Mat color, std::vector<cv::Mat> color_bg, cv::Scalar trans_th = cv::Scalar(255,15,15), int space_transformation=CV_BGR2Lab, int erode=1, int dilate=2, int min_connected_components=300);
