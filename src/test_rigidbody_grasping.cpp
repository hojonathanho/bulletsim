...
struct CustomScene : public Scene {
    ...
    void run() {
        ...
        ms.reset(new btDefaultMotionState(btTransform(btQuaternion(0, 0, 0, 1),
                 CFG.scene.scale * btVector3(0.7, 0.188, table_height + 0.1/2))));
        BoxObject::Ptr box(new BoxObject(1, CFG.scene.scale * btVector3(.03, .03, .03), ms));
        env->add(box);
        ...
        pr2Left->moveByIK(btTransform(btQuaternion(-0.250283, 0.967325, -0.00955494, 0.0393573),
                    CFG.scene.scale * btVector3(13.9589/20, 3.6742/20, 10.4952/20)));
