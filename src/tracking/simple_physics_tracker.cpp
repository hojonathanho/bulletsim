...
void SimplePhysicsTracker::doIteration() {
  ...
  if (m_enableObsInlierPlot) plotObs(toBulletVectors(m_obsPts.leftCols(3)), inlierFrac, m_obsInlierPlot);
	...
	if (m_enableEstPlot) plotNodesAsSpheres(m_obj->getColors(), vis, m_stdev, m_estPlot);
	else m_estPlot->clear();
	if (m_enableEstTransPlot) plotNodesAsSpheres(m_obj->getFeatures(FEAT_LAB), vis, m_stdev, m_estTransPlot);
	else m_estTransPlot->clear();
	if (m_enableCorrPlot) drawCorrLines(m_corrPlot, toBulletVectors(m_obj->getFeatures(FEAT_XYZ)), toBulletVectors(m_obsPts.leftCols(3)), pZgivenC, 0.01, m_count);
