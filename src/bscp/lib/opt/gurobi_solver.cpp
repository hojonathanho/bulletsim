...
void convex_gurobi_solver(const vector<VectorXd>& X_bar, const vector<VectorXd>& U_bar, const vector<MatrixXd>& W_bar,
  ...
  VectorXd x0, x_bar, u_bar, x0_T1, p_bar;
  MatrixXd W_s; 
  ...
  for (int t = 0; t < T+1; t++) {
	  for (int i = 0; i < NX; i++) {
		  lb[ind + t*NX + i] = X_lower_limit[t][i];
		  ub[ind + t*NX + i] = X_upper_limit[t][i];
	  }
  }
  ...
  for (int t = 0; t < T; t++) {
	  for (int i = 0; i < NU; i++) {
		  lb[ind + t*NU + i] = U_lower_limit[t][i];
		  ub[ind + t*NU + i] = U_upper_limit[t][i];
	  }
  }
