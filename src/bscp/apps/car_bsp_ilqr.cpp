...
int main()
{
  ...
  cout << "initial trajectory set" << endl;

  vector<vector<VectorXd> > W_s_t;
  index_by_sample(W_bar, W_s_t);
//  vector<VectorXd> test;
//  for (int s = 0; s < NS; s++) { THIS BE BROKEN BECAUSE W_S_T IS THE IDENTITY DISTRIBUTION
...
//  }
  ...
  ilqr_solver(c, B_bar, U_bar, W_bar, rho_x, rho_u, b_goal.segment(0,NX), N_iter,
      opt_B, opt_U, Q, r);
