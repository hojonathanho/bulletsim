...
int main(int argc, char *argv[]) {
    ...
    BoxObject::Ptr box(new BoxObject(0.1, btVector3(0.1, 0.1, 0.1), btTransform(btQuaternion(0, 0, 0, 1))));
    scene.env->add(box);

    SphereObject::Ptr sphere(new SphereObject(1, 0.1, btTransform(btQuaternion(0, 0, 0, 1))));
