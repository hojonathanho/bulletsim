...
public:
	typedef boost::shared_ptr<PlotLineStrip> Ptr;
	PlotLineStrip(float width=5);
	void setPoints(const std::vector<btVector3>& pts, const std::vector<btVector4>& cols);
	void setPoints(const std::vector<btVector3>& pts);
	void setPoints(const osg::ref_ptr<osg::Vec3Array>& pts, const osg::ref_ptr<osg::Vec4Array>& cols);
	void setPoints(const osg::ref_ptr<osg::Vec3Array>& pts);
...
/**  holds a set of lines together */
class PlotLinesSet : public EnvironmentObject {
public:
	typedef boost::shared_ptr<PlotLinesSet> Ptr;
	std::vector<PlotLineStrip::Ptr> l_sets;

	float lwidth;
	float r,g,b,a;

	PlotLinesSet(float width=5);
	...
	void clear() {
		for (int i =0; i < l_sets.size(); i++) {
			l_sets[i]->clear();
			getEnvironment()->remove(l_sets[i]);

		}
		l_sets.clear();
	}

	EnvironmentObject::Ptr copy(Fork &f) const { return Ptr(new PlotLinesSet(*this)); }

	void setDefaultColor(float _r, float _g, float _b, float _a) {
		r = _r; g = _g; b = _b; a = _a;
		for (int i =0; i < l_sets.size(); i++)
			l_sets[i]->setDefaultColor(r,g,b,a);
	}

	void addLineSet(const std::vector<btVector3> &pts);
	void addLineSet(const std::vector<btVector3> &pts, std::vector<btVector4> &cols);
