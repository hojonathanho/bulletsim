...
class RopeSceneEventHandler : public osgGA::TrackballManipulator {
private:
  RopeScene *scene;
...
public:
  RopeSceneEventHandler(RopeScene *scene_) : scene(scene_), state() {}
  struct {
    bool debugDraw,
...
struct RopeScene {
  typedef boost::shared_ptr<RopeScene> Ptr;
  ...
  osg::ref_ptr<RopeSceneEventHandler> manip;
